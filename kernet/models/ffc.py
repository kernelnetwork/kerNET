...
class _baseFFC(torch.nn.Module):
    ...
    def forward(self, x, upto=None, update_X=False):
        ...
        for i in range(upto):
            ...
            if not isinstance(layer, _kn): update_X=False
            if isinstance(layer, _kn): update_X=original_update_X

            if update_X and i==0:
                if isinstance(layer, knFCStack): 
                    # for stack layer
                    ...
                    for j in range(layer._comp_counter):
                        ...
                        if j > 0:
                            ...
                            if isinstance(comp, knFCEnsemble): 
                            # for ensemble layer
